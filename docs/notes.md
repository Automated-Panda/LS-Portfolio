...
- [~] **Phase 1b — catalogue data (partly done).** ✅ Sparrow id confirmed (`seasparrow2`). ✅ Migration `0040` seeded `vehicle_upgrades` + bays for the 3 existing containers: **Terrorbyte** (`terbyte` — Oppressor Mk II gated by Specialized Workshop; + Drone Station/Weapon Workshop/Turret/MCT), **Kosatka** (moon pool: Sparrow + **Kraken Avisa**; + Sonar/Guided Missiles), **Acid Lab** (`brickade2` — Manchez Scout C `manchez2`; + Equipment Upgrade). `lib/containers.ts` corrected (`terbyte`/`seasparrow2`).
  - ⏳ **Still needs James:** (1) **MOC** — needs a new catalogue vehicle row (class/price + **cab-choice decision**: mutex cosmetic upgrade or omit?) before its Vehicle-Workshop-gated bays can be seeded; (2) **Freakshop** property row (subtype `freakshop`, special) — can add now, **image** via temp-images later; (3) **images** for MOC + Freakshop.
